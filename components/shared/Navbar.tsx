...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 z-50 py-6 transition-all duration-300">
      <div className="container mx-auto px-2.5 sm:px-4 md:px-6 flex items-center justify-between relative">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 sm:gap-3 md:gap-4">
          <Image
            ...
            width={300}
            height={70}
            className="w-[140px] h-auto sm:w-[180px] md:w-[220px] lg:w-[300px] object-contain"
            priority
          />
        </Link>
        ...
        <button
          type="button"
          className={`lg:hidden ${mobileToggleColor} p-1.5 sm:p-2 rounded-md relative z-50`}
            ...
            <div className="flex items-center justify-between p-4 sm:p-5 md:p-6 border-b border-gray-200">
              ...
              >
                <Image
                  ...
                  width={300}
                  height={70}
                  className="w-[120px] h-auto sm:w-[140px] md:w-[180px] object-contain"
                  priority
                />
              </Link>
              <button
                type="button"
                className="text-carent-text p-1.5 sm:p-2"
                onClick={() => setIsMobileMenuOpen(false)}
            ...
            <div className="flex-1 flex flex-col p-4 sm:p-5 md:p-6">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`text-xl sm:text-2xl font-medium py-4 sm:py-5 md:py-6 border-b border-gray-100 last:border-0 ${
                    pathname === link.href ||
                    (pathname === "/" && link.href === "/")
                      ? "text-carent-dark"
